...
(function main() {
  ...
  const { winningNumber, winningBoard } = playGame(winningNumbers, boards, totalBoards);
...
function playGame(winningNumbers, boards, totalBoards) {
  let winningNumber = 0;
  let winningBoard = [];
  winningNumbers.every((currentNumber) => {
    ...
    console.log(`Winning number: ${winningNumber}, winning boards: ${winningBoards.length} / ${totalBoards}`);
